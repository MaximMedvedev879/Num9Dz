...
void MainWindow::initializeTable(QTableView *table, size_t rows)
{
    ...
    model->setHeaderData(4, Qt::Horizontal, "v_2_i");
    model->setHeaderData(5, Qt::Horizontal, "u_i - v_i");
    model->setHeaderData(6, Qt::Horizontal, "v_2_i - v_i");
    model->setHeaderData(7, Qt::Horizontal, "|S|");
    model->setHeaderData(8, Qt::Horizontal, "h_(i-1)");
...
3) 'u_i'   - значение 'u' (точного решения) в момент 'x_i'.\n\
4) 'v_i'   - значение 'v' в точке 'x_i', которое было принято методом.\n\
5) 'v_2_i' - значение 'v' в точке 'x_i', полученное из точки 'x_(i-1)' двойным счётом с половинным шагом.\n\
  На 0-м шаге метода полагается равным 'u0' по определению.\n\
6) 'u_i - v_i'    - глобальная погрешность в момент 'x_i'.\n\
7) 'v_2_i - v_i' - разность 'v_2_i' и 'v_i'.\n\
8) '|S|'      - '|S| = |v_2_i - v_i|/15' - величина, оцениваемая при принятии решения о продолжении счёта.\n\
9) 'h_(i-1)' - i >= 1: шаг численного интегрирования, 'h_(i-1) = x_i - x_(i-1)';\n\
                i = 0: начальный шаг интегрирования.\n\
10) 'C+' - количество удвоений шага интегрирования, выполненных на предыдущем шаге метода.\n\
11) 'C-'  - количество делений вдвое шага интегрирования, выполненных на предыдущем шаге метода.\n\
");
...
void MainWindow::on_pushButton_pressed()
{
    ...
    // Заполнение таблицы
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            model->setData(model->index(i,6), solution[i].v2 - solution[i].v);
            model->setData(model->index(i,7), solution[i].s);
    ...
    // График численного решения
        ...
        QCPItemText *tLabel = new QCPItemText(plot);
        tLabel->position->setCoords(0.0, u0);
        tLabel->setText(QString("(%1, %2, %3)").arg(a).arg(b).arg(u0));
        tLabel->setPen(Qt::NoPen);
        tLabel->setSelectedPen(Qt::NoPen);
        tLabel->setFont(QFont(font().family(), 12));
        tLabel->setColor(Qt::darkMagenta);
        ...
        solutionInfoString += "\nε_max = " + QString::number(v_eps) + ", ε_min = " + QString::number(v_eps/32.0);
...
>>>>>>> a7a0e1b9d63f62df418160488f152a794e46c4b2
